...
var Airpuck = {
	...
	Table: class Table {
		...
		getLocalRecordById(id) {
			let record = Airpuck.getLocalRecord.byId(this, id);
			return record;
		}
		...
		getLocalRecordListbyField(field, value, mode) {
			let recordList = Airpuck.getLocalRecord.ListbyField(this, field, value, mode);
			return recordList;
		}
		...
		getRemoteRecordById(id, callback) {
			if (this.currentRecord) {
				this.currentRecord = "";
			}
			Airpuck.getRemoteRecord.byId(this, id, callback);
		}
	...
	add: function (table, record, callback) {
		...
		xhr.onload = function () {
			if (xhr.status === 200) {
				...
				Airpuck.getRemoteRecord.byRecord(table, JSON.parse(xhr.response), function () {
					table.records.push(table.currentRecord);
					Airpuck.then(callback);
				});
	...
	remove: function (table, record, callback) {
		...
		xhr.onload = function () {
			if (xhr.status === 200) {
				table.records.forEach(function (record, index) {
					if (record.id == (JSON.parse(xhr.response)).id) {
	...
	getLocalRecord: {
		...
		ListbyField: function (table, field, value, mode) {
			var records = table.records;
			var recordList = [];
			for (var i in records) {
				var temp = records[i];
				if (mode === "Search") {
					...
					if (temp.fields[field].toUpperCase().includes(value.toUpperCase())) {
						recordList.push(temp);
					}
				}
